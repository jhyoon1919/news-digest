...
function scoreArticle(questionTokens, article) {
  const articleTokens = new Set(tokenize(`${article.title} ${article.summary} ${article.category} ${article.region}`));
  let score = 0;
...
module.exports = async (req, res) => {
  ...
  let articles = [];
  try {
    const url = `${SUPABASE_URL}/rest/v1/news_digest_articles?select=digest_date,region,category,title,summary,outlet,published_at,source_url&order=digest_date.desc&limit=300`;
  ...
  const contextText = contextArticles
    .map((a, i) => `[${i + 1}] (${a.digest_date}, ${a.region}/${a.category}, ${a.outlet}) ${a.title}\n${a.summary}\n원문: ${a.source_url}`)
    .join('\n\n');

  const systemPrompt = '당신은 저장된 뉴스 다이제스트 기사만 근거로 답변하는 도우미입니다. 아래 제공된 기사 목록에 없는 내용은 추측하지 말고, 목록에서 답을 찾을 수 없으면 "관련 기사를 찾지 못했습니다"라고 답하세요. 답변에는 근거로 삼은 기사의 날짜와 언론사를 함께 언급하세요.';
